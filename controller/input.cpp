...
InputDevice::InputDevice(FeedbackReceived feedbackReceived) : feedbackReceived(feedbackReceived) {
...
void InputDevice::addFeedback(uint16_t code) {
    if (ioctl(file, UI_SET_EVBIT, EV_FF) < 0 || ioctl(file, UI_SET_FFBIT, code) < 0) {
...
void InputDevice::create(std::string name, DeviceConfig config) {
    ...
    if (ioctl(file, UI_DEV_SETUP, &setup) < 0 || ioctl(file, UI_DEV_CREATE) < 0) {
...
void InputDevice::emitCode(uint16_t type, uint16_t code, int32_t value) {
    input_event event = {};
...
void InputDevice::handleFeedbackUpload(uint32_t id) {
    ...
    if (ioctl(file, UI_BEGIN_FF_UPLOAD, &upload) < 0) {
        Log::error("Error beginning feedback upload: %s", strerror(errno));
    ...
    if (ioctl(file, UI_END_FF_UPLOAD, &upload) < 0) {
        Log::error("Error ending feedback upload: %s", strerror(errno));
    }
}
...
void InputDevice::handleFeedbackErase(uint32_t id) {
    ...
    if (ioctl(file, UI_BEGIN_FF_ERASE, &erase) < 0) {
        Log::error("Error beginning feedback erase: %s", strerror(errno));
    ...
    if (ioctl(file, UI_END_FF_ERASE, &erase) < 0) {
        Log::error("Error ending feedback erase: %s", strerror(errno));
    }
}

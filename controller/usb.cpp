...
UsbDevice::UsbDevice(libusb_device *device, Terminate terminate) : terminate(terminate) {
    ...
    m_EPOUT = config->interface[0].altsetting[0].endpoint[0];
    m_EPIN  = config->interface[0].altsetting[0].endpoint[1];
...
void UsbDevice::controlTransfer(ControlPacket packet, bool write) {
    ...
    int transferred = libusb_control_transfer(
        handle, LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_RECIPIENT_DEVICE | direction, packet.request,

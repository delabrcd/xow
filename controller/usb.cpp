...
UsbDevice::UsbDevice(libusb_device *device, Terminate terminate) : terminate(terminate) {
    ...
    int c = 0;
    error = libusb_get_configuration(handle, &c);
    if (error) {
        throw UsbException("Error getting configuration", error);
    }
    ...
    error = libusb_get_config_descriptor(dev, 0, &config);
    if (error) {
        throw UsbException("Error getting usb config", error);
    }

    m_EPOUT = config->interface[0].altsetting[0].endpoint[1];
    m_EPIN  = config->interface[0].altsetting[0].endpoint[0];
    Log::debug("EndpointInAddress:    %02xh\n", m_EPIN.bEndpointAddress);
    Log::debug("EndpointOutAddress:    %02xh\n", m_EPOUT.bEndpointAddress);
...
UsbDevice::~UsbDevice() {
    ...
    if (error) {
        Log::error("Error releasing interface: %s", libusb_error_name(error));
    }
...
void UsbDevice::controlTransfer(ControlPacket packet, bool write) {
    ...
    int transferred =
        libusb_control_transfer(handle, packet.type, packet.request, packet.value, packet.index,
...
std::unique_ptr<UsbDevice> UsbDeviceManager::getDevice(std::initializer_list<HardwareId> ids,
    ...
    for (HardwareId id : ids) {
        int error = libusb_hotplug_register_callback(
            ...
            LIBUSB_HOTPLUG_ENUMERATE, id.vendorId, id.productId, LIBUSB_HOTPLUG_MATCH_ANY,
            hotplugCallback, &device, &handles[counter]);
    ...
    return std::unique_ptr<UsbDevice>(new UsbDevice(device, terminate));
}

int UsbDeviceManager::hotplugCallback(libusb_context *context, libusb_device *device,
